...
// called from shell
if(process.argv[1].match(/cmdRun.js/g)) {
		...
		_temp_base_complete() {
			...
			if [[ ${PRV} != "get" ]]; then
				...
				if [[ ${#COMP_WORDS[@]} -ge 1 ]]; then
					ARRAY=($(./cmdRun.js complete "${CMDFILE}" "${COMP_WORDS[@]:1:${#COMP_WORDS[@]}-2}" 2>/dev/null | tr -d '\r')) # handle CRLF in tty
				else
					ARRAY=($(./cmdRun.js complete "${CMDFILE}" 2>/dev/null | tr -d '\r')) # handle CRLF in tty
				fi
